...
const EditTrainsPage = ({ update, setUpdate }) => {
  const { trainId } = useParams();
    const apiUrl = import.meta.env.VITE_API_BASE_URL;
    const { refetch: refetchTrainData, loading: loadingTrainData, data: TrainData } = useGet({ url:`${apiUrl}/agent/train/item/${trainId}` });
    const { refetch: refetchTrainList, loading: loadingTrainList, data: trainList } = useGet({ url: `${apiUrl}/agent/train` });
    const { postData, loadingPost, response } = usePost({ url: `${apiUrl}/agent/train/update/${trainId}` });
    const auth = useAuth();
  ...
  useEffect(() => {
   ...
   useEffect(() => {
      if (TrainData && TrainData.train) {
        ...
        setSelectedTrainType(train.type_id || '');
        setSelectedTrainClass(train.class_id || '');
        setSelectedTrainRoute(train.route_id || '');
        setSelectedCountry(train.country_id || '');
        setStatus(train.status === 1? 'active' : 'inactive');
      }
  ...
  const handleSubmit = async (e) => {
    ...
    const data = {
      ...
      postData(data, 'Train Updated Success');
  };
