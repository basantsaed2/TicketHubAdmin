...
const EditCarPage = ({ update, setUpdate }) => {
    const apiUrl = import.meta.env.VITE_API_BASE_URL;
    const { carId } = useParams();
    const { refetch: refetchCarData, loading: loadingCarData, data: CarData } = useGet({ url:`${apiUrl}/agent/car/item/${carId}` });
    const { refetch: refetchCarList, loading: loadingCarList, data: carList } = useGet({ url:`${apiUrl}/agent/car` });
    const { postData, loadingPost, response } = usePost({ url: `${apiUrl}/agent/car/update/${carId}` });
    const auth = useAuth();
  ...
  useEffect(() => {
    refetchCarList();
    refetchCarData();
  }, [refetchCarList,refetchCarData, update]);
  ...
  useEffect(() => {
    if (CarData && CarData.car) {
        ...
        setSelectedCategory(car.category?.id || '')
        setSelectedModel(car.model?.id || '')
        setCarColor(car.car_color || '')
        setCarCapacity(car.capacity || 0)
        setCarYear(new Date(car.car_year) || '')
        setCarNumber(car.car_number || '')
        setStatus(car.status || '')
        setImageFile(car.image_link || '')
        setImageName(car.image || '')
    }
    console.log('CarData', CarData)
    }, [CarData]); // Only run this effect when `data` changes
  ...
  const handleSubmit = async (e) => {
    ...
    const data = {
      ...
      postData(data, 'Car Updated Success');
